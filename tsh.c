...
int builtin_cmd(char **argv) 
{
	...
	// see if this is a built in command, work accordingly
	//updated quit command to check for stopped jobs
...
void do_bgfg(char **argv) 
{
	...
	/*If the state is ST and the command is fg, change the state to FG, send SIGCONT signal to the process group*/
	if(!strcmp("fg", argv[0])) {
		if(job->state==ST){
			job->state = FG;
			kill(-(job->pid), SIGCONT);
...
{  
	int pid = fgpid(jobs);  
	int jid = pid2jid(pid);
    	// forward sigint to foreground jobs
	if (pid != 0) {  
		kill(-pid, SIGINT);
		if (sig < 0){  
...
{    
	...
	// forward sigtstp to foreground jobs
	if (pid != 0) { 
